...
class ParcelGen:
    ...
    CLASS_STR = "public abstract class %s implements %s {"
    ...
    def gen_getter(self, typ, member):
        ...
        return "    public %s %s() { return %s; }" % (typ, method_name, self.memberize(member))

    def gen_setter(self, typ, member):
        method_name = "set%s%s" % (member[0].capitalize(), member[1:])
        return "    @JsonProperty(\"%s\")\n    public void %s(%s value) { %s = value; }" % (self.get_json_for_member(member), method_name, typ, self.memberize(member))
    ...
    def gen_list_unparcel(self, typ, memberized):
        ...
        if (classname == "String"):
            ...
            # A classloader is not needed to read primitive or wrapper types
            return self.tabify("%s = (List<%s>) source.readValue(null);" % (memberized, classname))
        else:
    ...
    def print_child(self, child_name, package, super_class_import=None):
        self.tablevel = 0
        ...
        if super_class_import:
            imports.append(super_class_import)
    ...
    def print_gen(self, props, class_name, package, imports, transient):
        ...
        for typ, member in self.member_map():
            if member in transient:
                typ = "transient " + typ
            self.printtab("@JsonProperty(\"%s\") protected %s %s;" % (self.get_json_for_member(member), typ, self.memberize(member)))
        self.output("")
    ...
    if output:
        if (os.path.isdir(output)): # Resolve file location based on package + path
            ...
            if class_name.startswith("Gen"):
                child = class_name[3:]
                # Assume that the corresponding class should be one package
                # above the GenClass one. Eg.: GenFoo at com.example.gen.GenFoo
                # and Foo at com.example
                new_dirs = package.split(".")[:-1]
                new_package = '.'.join(new_dirs)
                new_dirs.append(child + ".java")
                super_class_import = package + "." + class_name
                child_file = os.path.join(output, *new_dirs)
                if not os.path.exists(child_file):
                    generator.outfile = open(child_file, 'w')
                    generator.print_child(child, new_package, super_class_import)
